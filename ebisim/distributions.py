...
class ChargeDistributions:
    ...
    def n_i_boltzmann(self, r, y):
        ...
        Returns the Boltzmann charge distribution for the ions [C.m-3].
                the derivative of this distribution by y.
        ...
        q_threshold = np.arange(self._element.z + 1)[(N > MINIMAL_DENSITY) & (kbT > MINIMAL_KBT)]
        # 2d-array of charge densities (and derivative) with CS as columns
        rho_partial = np.zeros((y.size, self._element.z + 1))
        rho_p_partial = np.zeros((y.size, self._element.z + 1))
        for i in q_threshold:
            rho_partial[:, i] += N[i] * Q_E * i * np.exp(-y * i / kbT[i])
            rho_p_partial[:, i] += - N[i] * Q_E * i**2 / kbT[i] * np.exp(-y * i / kbT[i])
        return [rho_partial.sum(axis=1), rho_p_partial.sum(axis=1)]
    ...
    def n_e_normal(self, *args):
        ...
        if isinstance(r, float) or isinstance(r, int):
            ...
            rho = -self._Q_e / (PI * self._re ** 2) * np.ones(r.size)
            rho[self._re < abs(r)] = 0
        rho_p = 0
        return np.array([rho, rho_p])

    @nb.jit
    def n_e_gaussian(self, *args):
        """
    ...
    def dict_charges_set(self, r, y, model):
        ...
        #ion_dict[model[0]] = getattr(self, 'n_i_' + model[0] + '_solver')(y, self._NkT, self._element.z)
        #electron_dict[model[1]] = getattr(self, 'n_e_' + model[1] + '_solver')(r, self._Q_e, self._re)
        ion_dict[model[0]] = n_i_boltzmann_s(y, self._NkT, self._element.z)
        electron_dict[model[1]] = n_e_gaussian_s(r, self._Q_e, self._re)
